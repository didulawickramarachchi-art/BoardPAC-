...
const roleLabel = role => ({ ADMIN: 'Admins', SECRETARY: 'Secretaries', MEMBER: 'Members' })[role]
...
export default function ResourcePage({ title, description, endpoint, createEndpoint, fields = [], columns, actions = [], editable = false, deletable = false, initialValues = {}, rowLink }) {
  ...
  const load = async () => { setLoading(true); setError(''); try { const { data } = await api.get(endpoint); setRows(Array.isArray(data) ? data : data.content || data.items || (data && typeof data === 'object' ? [data] : [])) } catch (e) { setError(errorMessage(e)) } finally { setLoading(false) } }
  useEffect(() => { load() }, [endpoint])
  const filtered = useMemo(() => rows
    .filter(row => JSON.stringify(row).toLowerCase().includes(search.toLowerCase()))
    .sort((a, b) => {
      if (title === 'Users') {
        const order = { ADMIN: 0, SECRETARY: 1, MEMBER: 2 }
        return order[roleCategory(a.role)] - order[roleCategory(b.role)] || String(a.username).localeCompare(String(b.username))
      }
      if (title !== 'Devices') return 0
      const pending = value => ['PENDING', 'REQUESTED', 'AWAITING_APPROVAL'].includes(String(value).toUpperCase())
      return Number(pending(b.status)) - Number(pending(a.status))
    }), [rows, search, title])
  ...
  const submit = async e => { e.preventDefault(); setSaving(true); setError(''); try { const body = editing ? Object.fromEntries(Object.entries(form).filter(([name]) => !fields.find(field => field.name === name)?.createOnly)) : form; editing ? await api.put(`${endpoint}/${editing.id}`, body) : await api.post(createEndpoint || endpoint, body); setModal(false); setEditing(null); setNotice('Record saved successfully.'); await load() } catch (err) { setError(errorMessage(err)) } finally { setSaving(false) } }
  ...
  const runAction = async (action, row) => { if (action.confirm && !window.confirm(action.confirm)) return; setError(''); try { const response = action.run ? await action.run(row, load) : await api[action.method || 'post'](action.path(row), typeof action.data === 'function' ? action.data(row) : action.data); setNotice(response?.data?.message || `${action.label} completed successfully.`); await load() } catch (err) { setError(errorMessage(err)) } }
